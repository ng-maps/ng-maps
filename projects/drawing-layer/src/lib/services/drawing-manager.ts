...
export class DrawingManager {
  ...
  protected _drawingManagers: Map<NgMapsDrawingLayer, Promise<google.maps.drawing.DrawingManager>> = new Map();
  ...
  async deleteDrawingManager(drawingManager: NgMapsDrawingLayer): Promise<void> {
  ...
  updateDrawingControl(drawingManager: NgMapsDrawingLayer): Promise<void> {
    return this._drawingManagers.get(drawingManager)
      .then((dm: google.maps.drawing.DrawingManager) => dm.setOptions({drawingControl: drawingManager.drawingControl}));
  }

  updateDrawingControlOptions(drawingManager: NgMapsDrawingLayer): Promise<void> {
    return this._drawingManagers.get(drawingManager)
      .then((dm: google.maps.drawing.DrawingManager) => dm.setOptions({drawingControlOptions: drawingManager.drawingControlOptions}));
  }

  updateCircleOptions(drawingManager: NgMapsDrawingLayer): Promise<void> {
    return this._drawingManagers.get(drawingManager)
      .then((dm: google.maps.drawing.DrawingManager) => dm.setOptions({circleOptions: drawingManager.circleOptions}));
  }

  updateMarkerOptions(drawingManager: NgMapsDrawingLayer): Promise<void> {
    return this._drawingManagers.get(drawingManager)
      .then((dm: google.maps.drawing.DrawingManager) => dm.setOptions({markerOptions: drawingManager.markerOptions}));
  }

  updatePolylineOptions(drawingManager: NgMapsDrawingLayer): Promise<void> {
    return this._drawingManagers.get(drawingManager)
      .then((dm: google.maps.drawing.DrawingManager) => dm.setOptions({polylineOptions: drawingManager.polylineOptions}));
  }

  updatePolygonOptions(drawingManager: NgMapsDrawingLayer): Promise<void> {
    return this._drawingManagers.get(drawingManager)
      .then((dm: google.maps.drawing.DrawingManager) => dm.setOptions({polygonOptions: drawingManager.polygonOptions}));
  }

  updateRectangleOptions(drawingManager: NgMapsDrawingLayer): Promise<void> {
    return this._drawingManagers.get(drawingManager)
      .then((dm: google.maps.drawing.DrawingManager) => dm.setOptions({rectangleOptions: drawingManager.rectangleOptions}));
  }

  createEventObservable<T>(eventName: string, drawingManager: NgMapsDrawingLayer): Observable<T> {
    return new Observable((observer: Observer<T>) => {
      this._drawingManagers.get(drawingManager).then((dm: google.maps.drawing.DrawingManager) => {
