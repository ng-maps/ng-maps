...
export class NgMapsDirectionDirective implements OnChanges, OnInit, OnDestroy {
  ...
  @Input() public markerOptions?: {
    ...
    waypoints?: Array<
      { infoWindow?: string | Node } & google.maps.MarkerOptions
    >;
  };
  ...
  @Output() public directionChange: EventEmitter<google.maps.DirectionsResult> =
    new EventEmitter<google.maps.DirectionsResult>();

  // Direction response for the new request
  @Output()
  public directionResponse: EventEmitter<google.maps.DirectionsResult> =
    new EventEmitter<google.maps.DirectionsResult>();

  // Send a custom infowindow
  @Output() public sendInfoWindow: EventEmitter<google.maps.InfoWindow> =
    new EventEmitter<google.maps.InfoWindow>();

  // Status of Directions Query (google.maps.DirectionsStatus.OVER_QUERY_LIMIT)
  @Output() public status: EventEmitter<google.maps.DirectionsStatus> =
    new EventEmitter<google.maps.DirectionsStatus>();
                ...
                try {
                  ...
                  if (typeof this.markerOptions.destination !== 'undefined') {
                    ...
                    this.markerOptions.destination.position =
                      _route.legs[_route.legs.length - 1].end_location;
                  ...
                  // Waypoints Marker
                  if (
                    typeof this.markerOptions !== 'undefined' &&
                    ...
                    for (let i = 0; i < _route.legs.length; i++) {
                      ...
                      if (i !== _route.legs.length - 1) {
                        ...
                        for (let j = 0; j < leg.via_waypoint.length; j++) {
                          ...
                          this.markerOptions.waypoints[addedWaypointIndex].map =
                            this.map;
                          this.markerOptions.waypoints[
                            addedWaypointIndex
                          ].position = via_waypoint.location;
                          this.waypointsMarker[addedWaypointIndex] =
                            this.setMarker(
                              this.map,
  ...
  private destroyMarkers(): void {
    ...
    try {
      if (
        typeof this.markerOptions !== 'undefined' &&
        typeof this.originMarker !== 'undefined'
      ) {
      ...
      if (
        typeof this.markerOptions !== 'undefined' &&
        typeof this.destinationMarker !== 'undefined'
      ) {
